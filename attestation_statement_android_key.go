...
func VerifyAndroidKeyAttestationStatement(
	...
	if err != nil {
		return nil, fmt.Errorf("%w: invalid attestation certificate extension, %s", ErrInvalidAttestationStatement, err)
	}
	if subtle.ConstantTimeCompare(keyDescription.AttestationChallenge, clientDataJSONHash[:]) != 1 {
		return nil, fmt.Errorf("%w: invalid attestation challenge", ErrInvalidAttestationStatement)
	}
	...
	if keyDescription.SoftwareEnforced.AllApplications ||
		...
		return nil, fmt.Errorf("%w: public key credential must be scoped to the RP id", ErrInvalidAttestationStatement)
	}
	...
	if keyDescription.TeeEnforced.Origin != android.KeyOriginGenerated {
		return nil, fmt.Errorf("%w: public key credential must be generated", ErrInvalidAttestationStatement)
	}

	// The value in the AuthorizationList.purpose field is equal to KM_PURPOSE_SIGN
	if !keyDescription.TeeEnforced.Purpose.Has(android.KeyMasterPurposeSign) {
		return nil, fmt.Errorf("%w: public key credential must support signing", ErrInvalidAttestationStatement)
	}
